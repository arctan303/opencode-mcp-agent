...
长任务应使用 `opencode_task_start(wait: false)`，然后轮询 `opencode_task_status`。`timeoutMs` 控制桥接器等待 OpenCode 的时间，但 MCP 客户端可能还有更短且独立的传输超时；使用异步模式可以避免客户端先在约 120 秒处中断。任务自身超时时，桥接器会返回 `OPENCODE_REQUEST_TIMEOUT` 并尝试 abort 对应 session，避免遗留半执行工具调用。
